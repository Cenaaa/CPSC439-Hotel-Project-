//
//  Registration.h : This class handles process of registration for a new account.
//  462_Hotel
...
private:
   ...
   void registrationFormInterface();
   void create_Temporary_Acct(GuestAccount &);
public:
   Registration();
...
/* registering(GuestAccount &):
   - Called in main.cpp to begin the account registration process.
...
void Registration::registering(GuestAccount &new_acct) {
   ...
   create_Temporary_Acct(new_acct);
}

/* registrationFormInterface():
   - Called by registering to obtain user profile information. Re-uses get_IDPW method of the Authentication class to obtain user's selected ID and password.
 parameter: None.
 Return: None
*/
void Registration::registrationFormInterface() {
...
/* create_acct(GuestAccount &new_acct):
   - Called by method - registering after profile information is obtained to initialize the empty GuestAccount object.
 Parameter: new_acct - of type GuestAccount passed-in by value.
 Return: None.
*/
void Registration::create_Temporary_Acct(GuestAccount &new_acct) {
...
/* update(DatabaseTree<GuestAccount> &, GuestAccount &):
   - Called in main.cpp to update the account database after sucessful initialization of new GuestAccount object.
 Parameter:
